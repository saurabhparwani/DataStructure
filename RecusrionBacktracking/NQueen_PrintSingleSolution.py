...
def solveNQueenUtility(board,col):
    ...
    for i in range(N):
        ...
        if isSafe(board,i,col):
            ...
            if solveNQueenUtility(board,col+1):
                return True

            board[i][col] = 'X'  # Backtrack if answer not Coming.

    return False  # Return This if we can not place Queen in any row.
